...
const неОткрылись = [];   // адрес не отдал 200 — разбираем после обхода
async function индексируемая(путь, тип, ждёмТипы) {
  проверено++;
  const адрес = путь;
  const r = await взять(путь);
  if (r.код !== 200) { неОткрылись.push({ адрес, код: r.код, ошибка: r.ошибка }); return; }
...
// Не открылись. Гид или страница спроса без вариантов честно отдаёт 404 и
// выпадает из sitemap — но узнаёт об этом сервер только при сборке страницы,
// то есть уже во время нашего обхода, и только если все площадки ответили.
// После запуска каталоги Check-in и Kvartirka собираются минут пятнадцать —
// до того пустоту не подтвердить, и адрес нарочно остаётся в sitemap (эту
// логику сторожит проверки/пустые-страницы.mjs). Здесь такое — примечание;
// нарушение — любой другой адрес не с кодом 200.
if (неОткрылись.length) {
  const свежая = new Set([...(await взять('/sitemap.xml')).тело.matchAll(/<loc>([^<]*)<\/loc>/g)].map(м => безОсновы(раскрыть(м[1]))));
  for (const н of неОткрылись) {
    if (н.код === 404 && пути.includes(н.адрес) && /^(гид|город и спрос)$/.test(типАдреса(н.адрес)))
      console.log('  (' + н.адрес + ' — 404: вариантов не хватило, '
        + (свежая.has(н.адрес) ? 'но ответили не все площадки — в sitemap пока остаётся)' : 'из sitemap выпала сама)'));
    else нарушение(н.адрес, 'код ' + н.код + (н.ошибка ? ' (' + н.ошибка + ')' : ''));
  }
}
