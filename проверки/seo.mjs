...
// то есть уже во время нашего обхода. Нарушение — если адрес и после этого
// остался в sitemap (или это не страница из sitemap вовсе).
if (неОткрылись.length) {
  ...
  for (const н of неОткрылись) {
    if (н.код === 404 && пути.includes(н.адрес) && !свежая.has(н.адрес) && /^(гид|город и спрос)$/.test(типАдреса(н.адрес)))
      console.log('  (' + н.адрес + ' — 404: вариантов не хватило, из sitemap выпала сама)');
    else нарушение(н.адрес, 'код ' + н.код + (н.ошибка ? ' (' + н.ошибка + ')' : '') + (свежая.has(н.адрес) ? ', и адрес всё ещё в sitemap' : ''));
  }
}
