...
const { закрыть } = запуститьChrome(PORT, 'tel');
...
// Две доски объявлений закрыты так же, как Realt.
for (const площадка of ['CheckIn', 'Kvartirka']) {
  const р = await js(`кнопкаТелефона({src:'` + площадка + `',phone:'375291234567',name:''},'call')`);
  check('у ' + площадка + ' телефон закрыт кнопкой',
    /^<button/.test(р || '') && !/tel:/.test(р || '') && /Показать телефон/.test(р || ''), р);
}
