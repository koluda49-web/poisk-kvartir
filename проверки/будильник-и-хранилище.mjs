...
const ворота = новыеВорота(), воротаСчётчика = новыеВорота(), воротаПовтора = новыеВорота(), воротаКонфликта = новыеВорота();
...
const В_GITHUB = {
  ...
  // первая отправка получает 409, а в GitHub тем временем другой экземпляр поднял b
  [путьК('конфликт')]:  { sha: 'k1', объект: { a: 10, b: 2 }, ждать: воротаКонфликта,
                          послеПервогоPUT: { sha: 'k2', объект: { a: 10, b: 7 } } },
  // первый ответ сразу (соединить бросит), повторы — только после ворот (второй раз тоже бросит)
...
const мок = http.createServer((req, res) => {
  ...
  req.on('end', async () => {
    ...
    } else if (req.method === 'PUT' && В_GITHUB[путь] && В_GITHUB[путь].послеПервогоPUT) {
      const ф = В_GITHUB[путь];
      Object.assign(ф, ф.послеПервогоPUT); delete ф.послеПервогоPUT;
      res.writeHead(409, { 'Content-Type': 'application/json' }); res.end('{"message":"is at k2 but expected k1"}');
...
try {
  ...
  check('соединить бросал дважды — в логе одна строка', лог.join('').split('\n').filter(с => /не слилось бросает/.test(с)).length === 1);

  // «конфликт»: слияние ещё не ушло, а GitHub уже другой (перекрытие при
  // развёртывании). Отправка получает 409, повторное слияние должно сохранить
  // и свою прибавку до загрузки, и чужое изменение.
  check('«конфликт» подключён', await подключить('конфликт', 'счётчик'));
  await дождаться(() => запросыК('GET', 'конфликт').length === 1);
  await пишем('конфликт', { a: 3 });   // +2 к местной копии
  воротаКонфликта.открыть();
  const конфликтИтог = { a: 12, b: 7 };   // своя прибавка к a и чужая к b
  check('после 409 данные в памяти: своя прибавка и чужое изменение', await дождаться(async () => same(await читаем('конфликт'), конфликтИтог)), JSON.stringify(await читаем('конфликт')));
  check('после 409 — вторая отправка', await дождаться(() => запросыК('PUT', 'конфликт').length === 2), 'PUT: ' + запросыК('PUT', 'конфликт').length);
  let ткф = {}, ткфПервая = {};
  try { ткфПервая = JSON.parse(запросыК('PUT', 'конфликт')[0].тело); ткф = JSON.parse(запросыК('PUT', 'конфликт')[1].тело); } catch {}
  const раскрыть = т => { try { return JSON.parse(Buffer.from(т.content || '', 'base64').toString('utf8')); } catch { return null; } };
  check('первая отправка — слитое с sha k1', same(раскрыть(ткфПервая), { a: 12, b: 2 }) && ткфПервая.sha === 'k1', JSON.stringify(раскрыть(ткфПервая)) + ' sha ' + ткфПервая.sha);
  check('вторая отправка — пересобранное с sha k2', same(раскрыть(ткф), конфликтИтог) && ткф.sha === 'k2', JSON.stringify(раскрыть(ткф)) + ' sha ' + ткф.sha);
  check('после 409 местный файл пересобран', same(файлом('конфликт'), конфликтИтог), JSON.stringify(файлом('конфликт')));
  ...
  // Строки про «../взлом», файл, которого нет в GitHub, «сбой», «бросает» и 409 у «конфликта» вызвала сама проверка.
  const ошибкиЛога = лог.join('').split('\n').filter(с => /Хранилище:|Будильник:/.test(с) && !/недопустимое имя|в GitHub ещё нет|не загрузилось сбой|не слилось бросает|не ушло конфликт — запись: ответ 409/.test(с));
  ...
  // Считаем только «при-выходе»: сервер сам подключает свои имена (предложения,
  // места от читателей), и подделка на их GET отдаёт {sha} без содержимого —
  // слитое с таким «файлом» тоже уходит при выходе, к этой проверке оно не относится.
  const putВыход = запросы.filter(q => q.method === 'PUT').slice(доВыхода)
    .filter(q => /при-выходе/.test(decodeURIComponent(q.url)));
