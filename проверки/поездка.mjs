...
try {
  ...
  if (естьКнопка) {
    ...
    const пары = (р.ссылка.match(/-?\d+\.\d+,-?\d+\.\d+/g) || []);
    check('в ссылке на карты все три точки (' + пары.length + ')', пары.length === 3, р.ссылка.slice(0, 120));
    check('ссылка ведёт в Яндекс.Карты', /yandex\.[a-z]+\/maps/.test(р.ссылка), р.ссылка.slice(0, 60));
