...
const { chrome, закрыть } = запуститьChrome(PORT, 'own');
...
// Деревня пишется «Осово»; «Осова» и «Оссово» — как её набирают люди, тоже должны находить.
for (const как of ['Осово', 'Осова', 'Оссово', 'осово'])
  check('костёл в Осово находится поиском «' + как + '»', (await найти(как)).some(p => p.id === 910029 && p.name === 'Костёл Святого Юрия (Георгия), Осово'));
