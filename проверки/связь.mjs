...
check('в блоке есть обращение к владельцам жилья', /Хотите разместить объявление/.test(текст));
check('первым предложено написать нам', /Напишите нам — и мы разместим/.test(текст),
      'пропало главное предложение — ради него блок и нужен');
check('сказано, что нужно для размещения', /фотографии/.test(текст) && /телефон/.test(текст),
      'человек не поймёт, что присылать');
check('площадки названы вторым способом', /Ещё один способ/.test(текст) && /Kufar/.test(текст));
