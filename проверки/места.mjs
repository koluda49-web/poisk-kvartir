...
console.log('\n=== свои снимки подхватываются по имени файла ===');
{
  ...
  for (const злой of ['/фото-точек/..%2F..%2Fpackage.json', '/фото-точек/..%5C..%5Ckvartiry-server.js']) {
    ...
    await r.text().catch(() => {});
    // Важно не какой именно код, а что файл не отдан: на Render такие адреса
    // отбивает ещё его собственный вход, кодом 400.
    check('попытка выйти из папки отбита (' + r.status + ')', r.status !== 200, злой);
  }
}
