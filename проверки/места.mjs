...
for (const [city, lat, lng, least] of [['Минск', 53.9023, 27.5619, 40], ['Брест', 52.0976, 23.7341, 10]]) {
  ...
  // Порядок: сперва разряд (храмы, дворцы, усадьбы, замки), потом расстояние.
  // Иначе в городе первым показывается ближайшая скульптура, а костёл
  // и усадьба уходят на второй экран.
  const ВЕРХ = ['Храмы', 'Дворцы и усадьбы', 'Укрепления'];
  const НИЗ = ['Культурные'];
  const катВерх = /костёл|костел|церк|храм|часовн|собор|кирха|синагог|монастыр|дворец|усадьб|замок|крепост/i;
  const катНиз = /скульптур|арт-объект|памятник|музей|скамейк/i;
  const разряд = p => ВЕРХ.includes(p.group) ? 0
                    : НИЗ.includes(p.group) ? 2
                    : (p.group && !['Строения','Военные','Ландшафтные','Разное'].includes(p.group)
                        ? (катВерх.test(p.cat || '') ? 0 : catНизТест(p) ? 2 : 1)
                        : 1);
  function catНизТест(p){ return катНиз.test(p.cat || ''); }
  const список = near.items || [];
  const поРазряду = список.every((x, i, a) => i === 0 || разряд(a[i - 1]) <= разряд(x));
  check(city + ': сперва храмы, дворцы и усадьбы', поРазряду,
        'разряды идут вперемешку: ' + список.slice(0, 6).map(p => разряд(p)).join(''));
  const вРазряде = список.every((x, i, a) =>
        i === 0 || разряд(a[i - 1]) !== разряд(x) || a[i - 1].km <= x.km);
  check(city + ': внутри разряда — от ближних к дальним', вРазряде);
  const первый = список[0];
  check(city + ': первым не арт-объект (' + ((первый || {}).cat || '—') + ')',
        !первый || разряд(первый) !== 2, 'наверху памятник или скульптура');
}
...
for (const [название, lat, lng] of [['Лидский замок', 53.8845, 25.3007],
                                    ...
                                    ['Коссовский замок', 53.1298, 25.1449]]) {
...
// ── 4в′. все виды жилья ──────────────────────────────
// У замка в глубинке ночуют в усадьбе или коттедже: квартир там может
// не быть вовсе. Поиск только по квартирам оставлял такие точки пустыми.
console.log('\n=== рядом ищутся не только квартиры ===');
{
  const всё = await json('/api/places/stay?lat=53.1298&lng=25.1449&r=30');
  const кв  = await json('/api/places/stay?lat=53.1298&lng=25.1449&r=30&type=flat');
  check('у Коссовского замка вариантов больше, чем одних квартир ('
        + (всё.total || 0) + ' против ' + (кв.total || 0) + ')',
        (всё.total || 0) > (кв.total || 0),
        'усадьбы и коттеджи не подмешиваются');
}

// ── 4в″. подбор не теряет жильё ──────────────────────
// Два числа, каждое из которых ловит свою половину решения.
// Заславль стоит на стыке города и области: если считать только по
// «своей» области, выходит около 455 вместо семисот.
// Лида: Kufar отдаёт не больше двухсот на запрос, и в областной выдаче
// её квартир помещается лишь треть — если не спросить город отдельно,
// выходит 30 вместо 76.
console.log('\n=== подбор не теряет жильё ===');
{
  const z = await json('/api/places/stay?lat=54.0030&lng=27.2760&r=30');
  check('Заславль: считается по всей стране, а не по одной области ('
        + (z.total || 0) + ')', (z.total || 0) > 600,
        'меньше шестисот — значит вернулись к подбору по областям');
  const l = await json('/api/places/stay?lat=53.8845&lng=25.3007&r=30');
  check('Лида: город спрошен отдельно (' + (l.total || 0) + ')', (l.total || 0) > 60,
        'меньше шестидесяти — значит спрашиваем только область');
}

// ── 4в‴. рядом ищутся все три источника ────────────────
// У Realt нет точных координат — метка стоит у центра города. Пока мы
// брали только точные, целый источник из трёх молча выпадал из подбора.
console.log('\n=== рядом ищутся все три источника ===');
{
  const источники = new Set();
  for (const [lat, lng] of [[53.9023, 27.5619], [52.4345, 30.9754],
                            [53.8940, 30.3310], [52.0976, 23.7341]]) {
    const d = await json('/api/places/stay?lat=' + lat + '&lng=' + lng + '&r=30');
    (d.items || []).forEach(x => источники.add(x.src));
  }
  for (const src of ['Kufar', 'Realt', 'Flatbook']) {
    check(src + ' попадает в подбор рядом', источники.has(src),
          'нашлись только: ' + [...источники].join(', '));
  }
}

// ── 4д. города с «ё» ─────────────────────────────────
// В нашем списке городов «Берёза» и «Рогачёв» пишутся через «ё»,
// у источников — через «е». Фильтр сравнивал буква в букву и отдавал ноль.
console.log('\n=== города с «ё» находятся ===');
for (const [город, обл] of [['Берёза', 'brest'], ['Рогачёв', 'gomel'], ['Могилёв', 'mogilev']]) {
  const r = await fetch(BASE + '/api/search?' + new URLSearchParams({ region: обл, city: город,
    type: 'flat', rooms: '', guests: '', max: '', source: 'both' }).toString());
  const d = await r.json();
  check(город + ': жильё находится (' + (d.total || 0) + ')', (d.total || 0) > 0,
        'ноль — значит сравниваем «ё» и «е» как разные буквы');
}

// ── 4е. кнопка ведёт в нужную область ────────────────────
// Область под кнопкой «Показать все варианты в области» должна быть той,
// где это жильё и стоит. Раньше бралась ближайшая по областному центру:
// Новогрудок и Барановичи открывали Минскую, Ружаны — Гродненскую.
console.log('\n=== кнопка ведёт в нужную область ===');
for (const [название, lat, lng, ждём] of [
  ['Новогрудок',  53.5996, 25.8255, 'grodno'],
  ['Барановичи',  53.1327, 26.0139, 'brest'],
  ['Браслав',     55.6400, 27.0410, 'vitebsk'],
  ['Ружаны',      52.8654, 24.8901, 'brest'],
  ['Полоцк',      55.4850, 28.7860, 'vitebsk'],
  ['Бобруйск',    53.1450, 29.2240, 'mogilev'],
  ['Минск',       53.9023, 27.5619, 'minsk'],
]) {
  const d = await json('/api/places/stay?lat=' + lat + '&lng=' + lng + '&r=30');
  check(название + ': область ' + ждём, d.region === ждём,
        'кнопка открывает «' + d.region + '»');
}
