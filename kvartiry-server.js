...
function точкаОбъявления(lat, lng, адрес, обл){
  ...
  if(!вБеларуси) return центр ? { lat: центр[0], lng: центр[1], approx: true }
                             : { lat: 0, lng: 0, approx: true };
  if(центр){
    if(distKm(lat, lng, центр[0], центр[1]) <= МЕТКА_ОТ_ГОРОДА_КМ) return { lat, lng, approx: false };
    return { lat: центр[0], lng: центр[1], approx: true, сверено: 'в центр города ' + место.имя };
  }
  if(обл){
...
function ciОбъявление(a, обл, названиеОбл){
  ...
  const естьВАдресе = т => !!т && сравнимо(город).includes(сравнимо(т));
  const хвост = место && (естьВАдресе(a.street_name) || естьВАдресе(a.house_number)) ? '' : адрес;
  ...
  return {
    ...
    name: a.owner_name || '',
    ...точкаОбъявления(+a.lat || 0, +a.lng || 0, город, обл),
...
function kvКарточки(html, обл, город, усадьбы, путьРаздела){
  ...
  for(const b of html.split('catalog__card-wrap').slice(1)){
    ...
    out.push({
      ...
      ...точкаОбъявления(к[0], к[1], усадьбы ? адрес : город, обл),
