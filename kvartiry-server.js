...
//      подключении (позже — последнее сверенное или отправленное). Разница
  ...
  if(пришло === null){
    ...
    if(з.соединить){
      try{
        ...
        console.log('Хранилище: не слилось ' + имя + ', ждём и пробуем снова —', e.message);
        повторитьПозже();
        return false;
      }
    }
...
//   бросает — первый вызов бросает исключение, дальше как обычное.
...
function подключитьПроверку(имя, вид){
  ...
  ДАННЫЕ_ПРОВЕРКИ[имя] = подключитьДанные(имя, {}, function(изGitHub, доЗагрузки, основа){
    ...
    if(вид === 'бросает' && !бросал){ бросал = true; throw new Error('проверка: слияние сломалось'); }
    if(вид === 'счётчик'){
