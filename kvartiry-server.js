...
async function сверить(имя){
  ...
  if(пришло === null){
    ...
    if(з.соединить){
      ...
      catch(e){ console.log('Хранилище: не слилось ' + имя + ', берём копию из GitHub —', e.message); }
    }
    const текст = JSON.stringify(итог, null, 1);
    записатьФайл(имя, текст);
    з.основа = текст;
    з.текст = JSON.stringify(итог) === JSON.stringify(пришло.объект) ? null : текст;
  }
...
// Для проверки хранилища (только DATA_TEST=1): имена из DATA_TEST_NAMES
// подключаются, как это сделает настоящий потребитель. Слияние — как у
// счётчиков: всё из GitHub, а записанное до загрузки добавляет свои ключи.
const ДАННЫЕ_ПРОВЕРКИ = {}, ОСНОВЫ_ПРОВЕРКИ = {};
if(process.env.DATA_TEST === '1'){
  (process.env.DATA_TEST_NAMES || '').split(',').filter(можноИмя).forEach(function(имя){
    ДАННЫЕ_ПРОВЕРКИ[имя] = подключитьДанные(имя, {}, function(изGitHub, доЗагрузки, основа){
      ОСНОВЫ_ПРОВЕРКИ[имя] = основа;
      ДАННЫЕ_ПРОВЕРКИ[имя] = Object.assign({}, доЗагрузки || {}, изGitHub);
      return ДАННЫЕ_ПРОВЕРКИ[имя];
    });
  });
}
