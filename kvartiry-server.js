...
// репозиторий, в папку данные/. «[skip render]» в сообщении не даёт такому
// коммиту запускать новое развёртывание. Отправляем не чаще раза в GH_SYNC_MS
// на файл: счётчики меняются часто, а коммит на каждое изменение — это шум.
//
// Откуда брать данные при запуске. Файл в папке данные/ приходит вместе с
// кодом развёртывания и почти всегда старше копии в GitHub: старый экземпляр
// успевает отправить свои последние изменения уже после того, как новый
// собрался. Поэтому при заданном GH_TOKEN правда — в GitHub.
//
// Правило для данных, которые сервер держит в памяти (подключитьДанные):
//   1. Сразу отдаём местную копию — сайт работает, не дожидаясь сети.
//   2. Тут же просим копию из GitHub. Пока она не пришла, записи ложатся
//      только в местный файл, в GitHub не уходят: иначе запись, сделанная по
//      старым данным, затёрла бы более новые.
//   3. Пришла копия — вызываем соединить(изGitHub, записаноДоЗагрузки, основа).
//      записаноДоЗагрузки — последнее, что потребитель записал и что ещё не
//      ушло (или undefined); это его полное состояние, а не изменения.
//      основа — от чего он отталкивался: местная копия, отданная при
//      подключении; после слияния — копия из GitHub, с которой сливали
//      (не итог: итог ещё не отправлен); после удачной отправки — то, что
//      ушло. Так при повторном слиянии (409) его изменения не теряются. Разница
//      записаноДоЗагрузки − основа и есть его изменения: счётчикам — прибавить
//      её к копии из GitHub, спискам — добавить новые записи. Потребитель
//      заменяет у себя данные на итог и возвращает его.
//      Вернул undefined или соединить нет — остаётся копия из GitHub, а
//      записи до загрузки отбрасываются.
//      соединить может вызываться больше одного раза (после 409 — снова
//      загрузка и слияние), поэтому он должен быть чистым: итог зависит
//      только от аргументов. Менять аргументы на месте можно — сравнение
//      с GitHub идёт по снимку, сделанному до вызова.
//      Бросил исключение — имя остаётся несверенным: строка в лог, файл и
//      память потребителя не трогаем, записи ждут, загрузка повторяется
//      раз в GH_SYNC_MS (иначе мы взяли бы копию из GitHub, а потребитель
//      остался бы со старыми данными в памяти).
//   4. Итог пишется в местный файл; если он отличается от копии в GitHub,
//      уходит в GitHub обычным порядком — с sha, полученным при загрузке.
//   5. В GitHub файла нет (404) — остаётся местная копия. Сеть или GitHub
//      не ответили — тоже местная копия, одна строка в лог, отправка ждёт,
//      загрузка повторяется раз в GH_SYNC_MS.
//   6. GitHub ответил «файл изменился» (409/422) — снова загружаем и сливаем.
// Имена, записанные без подключения, просто перезаписываются в GitHub.
...
const ХРАНИЛИЩЕ = { включено: !!(GH_TOKEN && GH_REPO), отправок: 0, последняяУдача: 0,
                    загружено: 0, ошибка: '', когдаОшибка: 0 };
const ОТЛОЖЕННОЕ = new Map();   // имя → записьИмени(имя)
...
const ошибкаХранилища = function(имя, текст){ ХРАНИЛИЩЕ.ошибка = имя + ': ' + текст; ХРАНИЛИЩЕ.когдаОшибка = Date.now(); };

function записьИмени(имя){
  let з = ОТЛОЖЕННОЕ.get(имя);
  if(!з){
    з = { текст: null,        // что отправить в GitHub (null — нечего)
          таймер: null,       // отложенная отправка
          повтор: null,       // повтор неудачной загрузки
          идёт: null,         // последняя операция с GitHub: они идут по очереди
          sha: undefined,     // sha файла в GitHub: undefined — не знаем, null — файла нет
          подключено: false,  // данные держат в памяти (подключитьДанные)
          сверено: false,     // копия из GitHub получена или её там нет
          основа: undefined,  // от чего отталкивается потребитель (текст JSON)
          соединить: null,
          ошибкаЗагрузки: false,
          ошибкаСлияния: false };
    ОТЛОЖЕННОЕ.set(имя, з);
  }
  return з;
}
// Операции с GitHub по одному имени идут строго друг за другом: две отправки
// разом взяли бы один sha, и одна получила бы 409.
function вОчередь(з, fn){
  const эта = (з.идёт || Promise.resolve()).then(fn).catch(function(e){
    console.log('Хранилище: сбой очереди —', e.message);
  });
  з.идёт = эта;
  return эта;
}
...
function записатьФайл(имя, текст){
  try{
...
function записатьДанные(имя, объект){
  ...
  if(!записатьФайл(имя, текст)) return false;
  if(ХРАНИЛИЩЕ.включено){ записьИмени(имя).текст = текст; поставитьОтправку(имя); }   // отправится самое свежее
  return true;
}

// Для данных, которые потребитель держит в памяти. Возвращает местную копию
// сразу; копию из GitHub передаст в соединить, когда она придёт (правило выше).
function подключитьДанные(имя, поУмолчанию, соединить){
  const местное = прочитатьДанные(имя, поУмолчанию);
  if(!можноИмя(имя) || !ХРАНИЛИЩЕ.включено) return местное;
  const з = записьИмени(имя);
  з.подключено = true;
  з.соединить = typeof соединить === 'function' ? соединить : null;
  з.основа = JSON.stringify(местное);
  вОчередь(з, function(){ return сверить(имя); });
  return местное;
}

function поставитьОтправку(имя){
  const з = записьИмени(имя);
  if(з.таймер || выходим) return;
...
const адресФайла = function(имя){
  return GH_API + '/repos/' + GH_REPO + '/contents/' + ['данные', имя + '.json'].map(encodeURIComponent).join('/');
};
const заголовкиGH = function(accept){
  return { 'Authorization': 'Bearer ' + GH_TOKEN, 'User-Agent': 'poisk-kvartir-server',
           'Accept': accept || 'application/vnd.github+json' };
};

// Копия из GitHub: { sha, объект } или null, если файла там нет.
async function изGitHub(имя){
  const адрес = адресФайла(имя) + '?ref=' + encodeURIComponent(GH_BRANCH);
  const г = await fetch(адрес, ждём({ headers: заголовкиGH() }));
  if(г.status === 404){ await г.text().catch(function(){}); return null; }
  if(!г.ok){ await г.text().catch(function(){}); throw new Error('чтение: ответ ' + г.status); }
  const д = await г.json();
  let текст;
  if(д.encoding === 'base64' && typeof д.content === 'string' && д.content) текст = Buffer.from(д.content, 'base64').toString('utf8');
  else{
    // Файлы больше мегабайта GitHub в этот ответ не кладёт — берём сырым.
    const с = await fetch(адрес, ждём({ headers: заголовкиGH('application/vnd.github.raw+json') }));
    if(!с.ok) throw new Error('чтение большого файла: ответ ' + с.status);
    текст = await с.text();
  }
  return { sha: д.sha, объект: JSON.parse(текст) };
}

// Загрузка и слияние; true — сверено (в том числе «файла в GitHub нет»).
async function сверить(имя){
  const з = записьИмени(имя);
  let пришло;
  const повторитьПозже = function(){
    if(з.повтор || выходим) return;
    з.повтор = setTimeout(function(){ з.повтор = null; вОчередь(з, function(){ return сверить(имя); }); }, GH_SYNC_MS);
    з.повтор.unref();
  };
  try{ пришло = await изGitHub(имя); }
  catch(e){
    ошибкаХранилища(имя, 'загрузка: ' + e.message);
    if(!з.ошибкаЗагрузки) console.log('Хранилище: из GitHub не загрузилось ' + имя + ', работаем с местной копией —', e.message);
    з.ошибкаЗагрузки = true;
    повторитьПозже();
    return false;
  }
  if(пришло === null){
    if(з.sha === undefined) console.log('Хранилище: в GitHub ещё нет ' + имя + ', остаётся местная копия');
    з.sha = null;
  }else{
    // Снимок до слияния: потребитель вправе менять изGitHub на месте
    // (счётчику так естественнее), и сравнивать итог с тем же объектом
    // значило бы всегда получать «ничего не изменилось» и не отправлять.
    const былоВGitHub = JSON.stringify(пришло.объект);
    const доЗагрузки = з.текст !== null ? JSON.parse(з.текст) : undefined;
    let итог = пришло.объект;
    if(з.соединить){
      try{
        const основа = з.основа === undefined ? undefined : JSON.parse(з.основа);
        const р = з.соединить(пришло.объект, доЗагрузки, основа);
        if(р !== undefined) итог = р;
      }catch(e){
        ошибкаХранилища(имя, 'слияние: ' + e.message);
        if(!з.ошибкаСлияния) console.log('Хранилище: не слилось ' + имя + ', ждём и пробуем снова —', e.message);
        з.ошибкаСлияния = true;
        повторитьПозже();
        return false;
      }
    }
    з.sha = пришло.sha;
    ХРАНИЛИЩЕ.загружено++;
    const текст = JSON.stringify(итог, null, 1);
    записатьФайл(имя, текст);
    // Основа — копия из GitHub, а не итог: итог ещё только ждёт отправки.
    // Получи отправка 409, повторное слияние возьмёт «итог − копия» как
    // изменения потребителя и приложит их к новой копии. С итогом в основе
    // разница вышла бы нулевой, и изменения до загрузки пропали бы.
    з.основа = былоВGitHub;
    з.текст = JSON.stringify(итог) === былоВGitHub ? null : текст;
  }
  з.сверено = true;
  // Сверено: отложенный повтор загрузки больше не нужен — он слил бы заново.
  if(з.повтор){ clearTimeout(з.повтор); з.повтор = null; }
  if(з.текст !== null) поставитьОтправку(имя);
  return true;
}

function отправитьИмя(имя){
  const з = записьИмени(имя);
  return вОчередь(з, async function(){
    // Подключённые данные не уходят, пока не сверены с GitHub (правило, п. 2).
    if(з.подключено && !з.сверено && !(await сверить(имя))) return;
    if(з.текст === null) return;
    const текст = з.текст; з.текст = null;
    try{
      await вGitHub(имя, текст, з);
      ХРАНИЛИЩЕ.отправок++; ХРАНИЛИЩЕ.последняяУдача = Date.now();
      з.основа = текст;
    }catch(e){
      ошибкаХранилища(имя, e.message);
      console.log('Хранилище: в GitHub не ушло ' + имя + ' —', e.message);
      if(e.конфликт){ з.sha = undefined; if(з.подключено) з.сверено = false; }
      // Не терять: если новее ничего не пришло, попробуем в следующем окне.
      if(з.текст === null) з.текст = текст;
      поставитьОтправку(имя);
    }
  });
}

async function вGitHub(имя, текст, з){
  const заголовки = заголовкиGH();
  // sha нужен, чтобы заменить уже лежащий файл. Знаем его после загрузки или
  // прошлой отправки; не знаем — спрашиваем, 404 значит «файла ещё нет».
  if(з.sha === undefined){
    const г = await fetch(адресФайла(имя) + '?ref=' + encodeURIComponent(GH_BRANCH), ждём({ headers: заголовки }));
    if(г.ok) з.sha = (await г.json()).sha;
    else{
  ...
  if(з.sha) тело.sha = з.sha;
  const п = await fetch(адресФайла(имя), ждём({ method: 'PUT',
    headers: Object.assign({ 'Content-Type': 'application/json' }, заголовки),
    body: JSON.stringify(тело) }));
  if(!п.ok){
    const e = new Error('запись: ответ ' + п.status + ' ' + (await п.text()).slice(0, 120));
    e.конфликт = (п.status === 409 || п.status === 422);   // файл в GitHub уже другой
    throw e;
  }
  // Новый sha из ответа: следующей отправке не нужен лишний запрос.
  try{ з.sha = (await п.json()).content.sha || undefined; }catch(e){ з.sha = undefined; }
}
...
function отправитьОтложенноеСейчас(){
  return Promise.all([...ОТЛОЖЕННОЕ.entries()].map(function(пара){
    ...
    if(з.повтор){ clearTimeout(з.повтор); з.повтор = null; }
    return з.текст !== null ? отправитьИмя(пара[0]) : з.идёт;
  }));
}

// Для проверки хранилища (только DATA_TEST=1): имена подключаются, как это
// сделает настоящий потребитель, — при запуске (DATA_TEST_NAMES) или по
// запросу проверки. Виды слияния:
//   обычное — всё из GitHub, записанное до загрузки добавляет свои ключи;
//   счётчик — на месте: к изGitHub прибавляется разница «записано − основа»;
//   бросает — первые два вызова бросают исключение, дальше как обычное.
const ДАННЫЕ_ПРОВЕРКИ = {}, ОСНОВЫ_ПРОВЕРКИ = {};
function подключитьПроверку(имя, вид){
  let бросал = 0;
  ДАННЫЕ_ПРОВЕРКИ[имя] = подключитьДанные(имя, {}, function(изGitHub, доЗагрузки, основа){
    ОСНОВЫ_ПРОВЕРКИ[имя] = основа;
    if(вид === 'бросает' && бросал < 2){ бросал++; throw new Error('проверка: слияние сломалось'); }
    if(вид === 'счётчик'){
      Object.keys(доЗагрузки || {}).forEach(function(к){
        изGitHub[к] = (изGitHub[к] || 0) + доЗагрузки[к] - ((основа || {})[к] || 0);
      });
      return (ДАННЫЕ_ПРОВЕРКИ[имя] = изGitHub);
    }
    return (ДАННЫЕ_ПРОВЕРКИ[имя] = Object.assign({}, доЗагрузки || {}, изGitHub));
  });
}
if(process.env.DATA_TEST === '1'){
  (process.env.DATA_TEST_NAMES || '').split(',').filter(можноИмя).forEach(function(имя){ подключитьПроверку(имя, 'обычное'); });
}
...
// У других событий id не храним: журналу он там ничего не говорит, а место занимает.
...
function statsFields(o){
  ...
  for(const k of T_FIELDS){
    if(k === 'id' && o.e !== 'route_add') continue;
    const v = o[k];
...
function statsPage(популярное){
  ...
  return '<!doctype html><html lang="ru"><head><meta charset="utf-8">' +
    ...
    '<h2>Постоянное хранилище</h2><div class="card"><div class="tiles">' +
      ...
      tile('Загружено из GitHub', ХРАНИЛИЩЕ.загружено, 'файлов при запуске') +
...
const TOWN_CENTERS = {
  ...
  // Жодино стояло в тридцати километрах к северо-западу, под Логойском:
  // сверка с OpenStreetMap при разборе адресов объявлений (15.09).
  'Жодино':[54.0985,28.3331],'Слуцк':[53.0270,27.5520],'Дзержинск':[53.6840,27.1440],'Вилейка':[54.4890,26.9190],
...
// Заголовок Realt. Город у них приходит дважды: в townName и в начале
// address — «Браслав» и «Браслав Ленинская ул. 76». Мы клеили одно к другому,
// и в карточке выходило «Браслав Браслав Ленинская ул. 76» — так было у всех
// пятисот объявлений. Город ставим впереди, только если адрес с него не начинается.
function заголовокRealt(город, текст){
  const т = String(текст || '').replace(/\s+/g, ' ').trim();
  const г = String(город || '').replace(/\s+/g, ' ').trim();
  if(!г) return т;
  const с = сравнимо(т), сг = сравнимо(г);
  const ужеЕсть = с === сг || (с.startsWith(сг) && /[^а-яa-z0-9-]/.test(с.charAt(сг.length)));
  return ужеЕсть ? т : (г + ' ' + т).trim();
}
  ...
  return cached('raw|realt|'+type+'|'+reg.realt, async ()=>{
    ...
    return res.map(a=>{
      ...
      return { src:'Realt',
        ...
        title:заголовокRealt(town, a.address||a.title),
...
const EXTRA_PLACES = [
 ...
 {
  "id": 910029,
  "name": "Костёл Святого Юрия (Георгия), Осово",
  "lat": 54.078594,
  "lng": 25.216469,
  "addr": "д. Осово, Вороновский р-н",
  "alt": "Осова, Оссово, Асава",
  "cat": "костёл",
  "group": "Из маршрутов",
  "pic": "",
  "text": "Каменный неоготический костёл 1909–1910 годов в деревне Осово. Рядом сохранилась старая плебания начала XX века. От Вороново около 10 км — удобно объединить в один день с костёлом Милосердия Божьего."
 },
 {
  "id": 910030,
 ...
 {
  "id": 910034,
  "name": "Костёл Святого Иоанна Крестителя (Гольшаны)",
  "lat": 54.25687,
  "lng": 26.00999,
  "addr": "д. Гольшаны, Ошмянский р-н",
  "alt": "церковь Иоанна Крестителя, францисканский монастырь",
  "cat": "костёл",
  "group": "Из маршрутов",
  "pic": "",
...
//
// Метка бывает и белорусской, но чужой. Квартира «Глубокое, ул. Октября 41»
// стояла на улице Октября в Браславе — в семидесяти километрах от Глубокого, —
// и «Жильё рядом» у Замковой горы показывало её в полутора километрах.
// Такое же было у Бреста и Могилёва с меткой в Минске, у Солигорска с меткой
// в Светлогорске. Поэтому метку сверяем с адресом.
//
// Для сверки нужны районные центры, которых нет в TOWN_CENTERS. Отдельной
// таблицей, а не в TOWN_CENTERS: по каждому городу оттуда «Жильё рядом»
// шлёт свой запрос к Kufar, и сотня лишних городов умножила бы запросы.
// Координаты — центры населённых пунктов из OpenStreetMap. На общих
// с TOWN_CENTERS городах расхождение было не больше трёх километров,
// кроме Жодино — там ошибались как раз TOWN_CENTERS, поправлено выше.
const РАЙЦЕНТРЫ = {
  'brest': {
    'Дрогичин':[52.185,25.160], 'Ивацевичи':[52.709,25.339], 'Каменец':[52.402,23.822],
    'Ляховичи':[53.037,26.266], 'Малорита':[51.792,24.084], 'Столин':[51.890,26.850],
    'Белоозёрск':[52.468,25.178], 'Микашевичи':[52.224,27.487] },
  'vitebsk': {
    'Бешенковичи':[55.046,29.458], 'Верхнедвинск':[55.783,27.936], 'Городок':[55.461,29.987],
    'Докшицы':[54.895,27.760], 'Дубровно':[54.572,30.690], 'Лиозно':[55.023,30.796],
    'Миоры':[55.622,27.615], 'Россоны':[55.904,28.814], 'Сенно':[54.814,29.708],
    'Толочин':[54.410,29.694], 'Ушачи':[55.179,28.617], 'Чашники':[54.857,29.160],
    'Шарковщина':[55.366,27.467], 'Шумилино':[55.299,29.617], 'Новолукомль':[54.657,29.142],
    'Барань':[54.479,30.316] },
  'gomel': {
    'Брагин':[51.781,30.275], 'Буда-Кошелёво':[52.716,30.565], 'Ветка':[52.559,31.171],
    'Ельск':[51.806,29.152], 'Житковичи':[52.217,27.855], 'Корма':[53.133,30.798],
    'Лельчицы':[51.786,28.329], 'Лоев':[51.936,30.800], 'Наровля':[51.798,29.507],
    'Октябрьский':[52.647,28.883], 'Петриков':[52.126,28.496], 'Хойники':[51.888,29.965],
    'Чечерск':[52.919,30.917], 'Туров':[52.072,27.730] },
  'grodno': {
    'Большая Берестовица':[53.193,24.017], 'Берестовица':[53.193,24.017], 'Вороново':[54.152,25.320],
    'Дятлово':[53.466,25.405], 'Зельва':[53.150,24.817], 'Ивье':[53.930,25.770],
    'Кореличи':[53.571,26.139], 'Мосты':[53.413,24.543], 'Свислочь':[53.036,24.094],
    'Щучин':[53.605,24.742], 'Скидель':[53.590,24.251], 'Мир':[53.454,26.466] },
  'minsk-obl': {
    'Березино':[53.835,28.994], 'Воложин':[54.089,26.526], 'Клецк':[53.066,26.633],
    'Копыль':[53.151,27.091], 'Крупки':[54.317,29.140], 'Любань':[52.792,28.000],
    'Мядель':[54.871,26.930], 'Несвиж':[53.219,26.682], 'Старые Дороги':[53.039,28.265],
    'Столбцы':[53.481,26.737], 'Узда':[53.465,27.215], 'Фаниполь':[53.748,27.328],
    'Нарочь':[54.937,26.690] },
  'mogilev': {
    'Белыничи':[53.998,29.710], 'Глуск':[52.899,28.686], 'Дрибин':[54.118,31.093],
    'Кировск':[53.273,29.469], 'Кличев':[53.490,29.342], 'Костюковичи':[53.352,32.058],
    'Краснополье':[53.336,31.401], 'Круглое':[54.248,29.797], 'Мстиславль':[54.020,31.726],
    'Славгород':[53.446,31.006], 'Хотимск':[53.408,32.579], 'Чаусы':[53.807,30.973],
    'Чериков':[53.570,31.381] },
};
// Минск и Минская область для сверки — одно: минские квартиры check-in.by
// лежат в разделе области.
const однаОбласть = (а, б) => а === б
  || ((а === 'minsk' || а === 'minsk-obl') && (б === 'minsk' || б === 'minsk-obl'));
// Все города для сверки: [имя, область, [широта, долгота]].
const ГОРОДА_СВЕРКИ = Object.entries(TOWN_CENTERS).map(([имя, к]) => [имя, регионГорода(имя), к])
  .concat(...Object.entries(РАЙЦЕНТРЫ).map(([обл, города]) =>
    Object.entries(города).map(([имя, к]) => [имя, обл, к])));
// Одноимённых городов много — Свислочь есть и под Гродно, и под Осиповичами,
// — поэтому город ищем только в области самого объявления.
function центрГорода(имя, обл){
  const и = сравнимо(имя);
  if(!и) return null;
  const г = ГОРОДА_СВЕРКИ.find(x => сравнимо(x[0]) === и && (!обл || однаОбласть(x[1], обл)));
  return г ? г[2] : null;
}

// Город из адреса: «г. Браслав, ул. Ленинская, 17» → Браслав, город;
// «д. Черевки, ул. Новая, 13» → Черевки, деревня. Деревню с одноимённым
// городом не путаем: «а/г Свислочь» — не город Свислочь.
const ПРИСТАВКА_ГОРОДА  = /^(?:г\.\s*п\.|гп\.|гп\s|р\.\s*п\.|г\.)\s*/i;
const ПРИСТАВКА_ДЕРЕВНИ = /^(?:аг\.|а\.\s*г\.|а\/г|д\.|п\.|пос\.|к\.\s*п\.|х\.|с\.)\s*/i;
function местоИзАдреса(адрес){
  const к = String(адрес || '').split(',').map(x => x.trim())
    .filter(x => x && !/област|р-н|район/i.test(x))[0] || '';
  if(ПРИСТАВКА_ГОРОДА.test(к)) return { имя: к.replace(ПРИСТАВКА_ГОРОДА, '').trim(), деревня: false };
  if(ПРИСТАВКА_ДЕРЕВНИ.test(к) && !/^д\.\s*\d/i.test(к))
    return { имя: к.replace(ПРИСТАВКА_ДЕРЕВНИ, '').trim(), деревня: true };
  // Без приставки — так check-in.by пишет свои города: «Глубокое», «Минск».
  if(/^[А-ЯЁ][а-яё]+(?:[\s-][А-ЯЁа-яё][а-яё]+)?$/.test(к)) return { имя: к, деревня: false };
  return { имя: '', деревня: true };
}

// Дальше тридцати километров от центра города из адреса — метка не от него.
const МЕТКА_ОТ_ГОРОДА_КМ = 30;
// До ближайшего райцентра своей области. По всем трём тысячам объявлений
// деревни стоят не дальше сорока километров; дальше пятидесяти были только
// метки из чужой области и из-за границы.
const МЕТКА_ОТ_ОБЛАСТИ_КМ = 50;

// Точка объявления после сверки с адресом. Город из адреса знаем, а метка
// далеко — ставим в центр города с пометкой «примерно»: в выдаче по своему
// городу объявление остаётся, на карте стоит у своего города. Город не
// знаем (деревня), а метка далеко от всех райцентров своей области — точку
// убираем: карточка в списке останется, но на карте и в «Жильё рядом» её
// не будет. Поле «сверено» говорит, что именно сделали, — его показывает
// /istochnik и читает проверка «адреса-объявлений».
// Перенесённые в центр объявления не кладём в одну точку — иначе на карте
// они слипаются в одну метку. Разброс как у Realt (approxCoord, около
// километра), но зерно — сама ссылка: цифр в ссылках check-in.by нет.
function околоЦентра(центр, ключ){
  let n = 2166136261;                                   // FNV-1a: соседние ссылки дают разные точки
  for(const ч of String(ключ || '')) n = Math.imul(n ^ ч.charCodeAt(0), 16777619) >>> 0;
  const dLat = (((n % 97) / 97) - 0.5) * 0.020;
  const dLng = ((((n >>> 8) % 97) / 97) - 0.5) * 0.032;
  return [ +(центр[0] + dLat).toFixed(6), +(центр[1] + dLng).toFixed(6) ];
}

function точкаОбъявления(lat, lng, адрес, обл, ключ){
  const место = местоИзАдреса(адрес);
  const центр = место.деревня ? null : центрГорода(место.имя, обл);
  const вБеларуси = lat > 51 && lat < 57 && lng > 23 && lng < 33;
  const около = центр ? околоЦентра(центр, ключ) : null;
  if(!вБеларуси) return около ? { lat: около[0], lng: около[1], approx: true }
                             : { lat: 0, lng: 0, approx: true };
  if(центр){
    if(distKm(lat, lng, центр[0], центр[1]) <= МЕТКА_ОТ_ГОРОДА_КМ) return { lat, lng, approx: false };
    return { lat: около[0], lng: около[1], approx: true, сверено: 'в центр города ' + место.имя };
  }
  if(обл){
    const ближе = ГОРОДА_СВЕРКИ.filter(x => однаОбласть(x[1], обл))
      .reduce((м, x) => Math.min(м, distKm(lat, lng, x[2][0], x[2][1])), Infinity);
    if(ближе > МЕТКА_ОТ_ОБЛАСТИ_КМ) return { lat: 0, lng: 0, approx: true, сверено: 'убрано с карты' };
  }
...
function ciОбъявление(a, обл, названиеОбл){
  ...
  // кончался висящим «…, ул.».
  const улица = a.street_name ? [a.street_type, a.street_name].filter(Boolean).join(' ') : '';
  const адрес = [улица, a.house_number, a.building].filter(Boolean).join(' ');
  const город = (a.city && a.city.name) || a.city_custom || '';
  // Свой адрес целиком лежит в city_custom. Городом карточки делаем только
  // название: «г. Браслав, пер. Мицкевича, 9» → «Браслав».
  const место = a.city && a.city.name ? null : местоИзАдреса(a.city_custom);
  // И улица с домом там обычно уже есть — второй раз их не приклеиваем.
  // Сверяем целыми словами: простое «содержит» находило дом «1»
  // в «12 км от Браслава», и улица с домом пропадали из заголовка.
  // Улицу — по названию; без улицы — номер дома отдельным словом.
  const слова = т => ' ' + сравнимо(т).replace(/[^а-яa-z0-9]+/g, ' ').trim() + ' ';
  const ужеЕсть = !!место && (a.street_name
    ? слова(город).includes(слова(a.street_name))
    : (!!a.house_number && слова(город).includes(слова(a.house_number))));
  const хвост = ужеЕсть ? '' : адрес;
  const ссылка = 'https://check-in.by/' + (a.type === 'house' ? 'dom/' : 'kvartira/') + a.slug;
  ...
  return {
    ...
    area: (место && место.имя) || город, region: названиеОбл, обл,
    capacity: a.guests_count || '',
    title: [город, хвост].filter(Boolean).join(', ') || (a.title || ''),
    photos: фото,
    rating: 0, reviews: 0, descId: null,
    phone: String(a.owner_phone || '').replace(/\D/g, ''),
    name: a.owner_name || '',
    ...точкаОбъявления(+a.lat || 0, +a.lng || 0, город, обл, ссылка),
...
function kvКарточки(html, обл, город, усадьбы, путьРаздела){
  ...
  for(const b of html.split('catalog__card-wrap').slice(1)){
    ...
    out.push({
      ...
      rating: 0, reviews: 0, descId: null,
      phone: тел[0] || '', name: '',
      // У усадеб «город» — это область, а сам населённый пункт стоит в адресе.
      ...точкаОбъявления(к[0], к[1], усадьбы ? адрес : город, обл, 'kvartirka' + путьРаздела + id),
...
// Повтор — это тот же посетитель (v) или тот же адрес: v придумывает сам
// браузер, и цикл запросов с новым v на каждый раз вытолкнул бы любое место
// наверх. С адресом так не выйдет. Цена — двое за одним домашним роутером,
// добавившие одно и то же место, дадут один голос; для «популярного» это не страшно.
async function голосЗаМесто(id, посетитель, робот, адрес){
  const к = номерМеста(id), v = String(посетитель || ''), ip = String(адрес || '');
  if(!к || !v || робот) return false;
  // Разные приставки: v пишет браузер, и без них v вида «ip 1.2.3.4» занял бы чужой адрес.
  const ключи = ['v ' + v + '|' + к].concat(ip ? ['ip ' + ip + '|' + к] : []);
  const повтор = () => ключи.some(function(x){ return ПОПУЛЯРНОЕ.голоса.has(x); });
  if(повтор()) return false;
  ...
  if(!есть || повтор()) return false;
  ключи.forEach(function(x){ ПОПУЛЯРНОЕ.голоса.add(x); });
  // Set помнит порядок добавления: первыми идут самые старые голоса.
  while(ПОПУЛЯРНОЕ.голоса.size > ГОЛОСОВ_МАКС) ПОПУЛЯРНОЕ.голоса.delete(ПОПУЛЯРНОЕ.голоса.values().next().value);
...
function рейсPage(d){
  ...
  return '<!doctype html><html lang="ru"><head><meta charset="utf-8">'
    ...
    + '<div class="g">'
    // Яндекс из дата-центра отвечает не всегда, поэтому время вылета берём
    // у него, а если он молчит — у самой авиакомпании: она его тоже знает.
    +   плитка('Вылет по плану', в.plan ? времяВ(в.plan, 4) : (а.planDep || '—'), 'Самара')
    +   плитка('Вылет ожидается',
              а.factDep || (в.actual || в.plan ? времяВ(в.actual || в.plan, 4) : (а.planDep || '—')),
              опоздание ? ('позже на ' + мин(опоздание))
                        : (в.status === 'on_time' || а.planDep ? 'без изменений' : ''))
...
// Снимок для превью в соцсетях и поиске — только наш (/фото-точек/…) или
// с kudin.by: так владелец решил про фотографии, которые расходятся по чужим
// лентам. Снимки со сторонних сайтов (commons.wikimedia.org у встроенных
// точек маршрутов) на странице показываются, а в превью не идут.
function годенДляПревью(pic){
  const п = String(pic || '');
...
// закрыл тег, а разделители строк U+2028/U+2029 — чтобы не порвали строку
// в старых браузерах.
function вСкрипт(v){
...
// добавляем местность из адреса — сначала населённый пункт, потом район
// (область и сельсовет — последними). Имя уже содержит местность («Усадьба
...
function частиИмениМеста(p){
  ...
  const вИмени = стебель.length >= 3
    && new RegExp('(^|[^а-яa-z0-9])' + стебель.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).test(имя.toLowerCase().replace(/ё/g, 'е'));
...
const нормИмени = s => String(s).toLowerCase().replace(/ё/g, 'е');

// Имя с местностью и различителем в пределе знаков. Длинное имя режем само,
// местность и различитель оставляем. Если одинаковыми стали обрезанные имена
// (редюиты Бобруйской крепости отличаются только концом названия), режем
// середину: «Редюит … между бастионами II и III, Бобруйск».
function собратьИмяМеста(p, предел, различитель, сКонцом){
  const ч = частиИмениМеста(p);
  let имя = ч.имя.charAt(0).toUpperCase() + ч.имя.slice(1), где = ч.где;
  const хвост = () => (где.length ? (', ' + где.join(', ')) : '') + (различитель || '');
  if(предел && (имя + хвост()).length > предел){
    ...
    if((имя + хвост()).length > предел && место >= 15){
      const слова = имя.split(' ');
      if(сКонцом && слова.length > 2){
        let конец = [];
        for(let i = слова.length - 1; i > 0; i--){
          if((слова[0] + ' … ' + [слова[i]].concat(конец).join(' ')).length > место) break;
          конец.unshift(слова[i]);
        }
        if(конец.length) имя = слова[0] + ' … ' + конец.join(' ');
      }
      if(имя.length > место){
...
// Различители одноимённых мест одной местности («Часовня», д. Дубок — две;
// «Дот» без адреса — девять). По порядку, пока место не станет единственным
// в своей группе: раздел справочника, если у соседей другой; годы постройки;
// короткое описание целиком («Дот Мозырского УР», «Возле музея "Дудутки"»);
// и только если данных нет — координаты. Всё — данные справочника, ничего
// не придумываем. Описания соседей берём из кэша placeDetail (сутки).
async function различительМеста(p, list){
  const основа = нормИмени(основаИмениМеста(p));
  const группа = (list || []).filter(x => нормИмени(основаИмениМеста(x)) === основа);
  if(группа.length < 2) return '';
  const подробно = await Promise.all(группа.map(x =>
    placeDetail(x.id).catch(() => ({ text: '', years: '' }))));
  const уровни = [
    (x, i) => x.cat ? (x.cat.charAt(0).toLowerCase() + x.cat.slice(1)) : '',
    (x, i) => String(подробно[i].years || '').trim(),
    (x, i) => {
      const т = короткоеОписание(подробно[i].text || '', 50);
      return (т && !/…$/.test(т) && нормИмени(т) !== нормИмени(x.name)) ? т.replace(/\.$/, '') : '';
    },
  ];
  const выбрано = группа.map(() => '');
  for(const уровень of уровни){
    const знач = группа.map(уровень);
    группа.forEach(function(x, i){
      if(выбрано[i] || !знач[i]) return;
      if(знач.filter(v => нормИмени(v) === нормИмени(знач[i])).length === 1) выбрано[i] = знач[i];
    });
  }
  const свой = группа.findIndex(x => String(x.id) === String(p.id));
  const повтор = выбрано.filter(v => v && нормИмени(v) === нормИмени(выбрано[свой])).length > 1;
  return (выбрано[свой] && !повтор)
    ? (' (' + выбрано[свой] + ')')
...
function имяМестаСМестностью(p, list, предел, различитель){
  ...
  const совпало = (list || []).some(x => String(x.id) !== String(p.id)
    && нормИмени(основаИмениМеста(x)) !== нормИмени(основаИмениМеста(p))
    && нормИмени(собратьИмяМеста(x, предел, '')) === нормИмени(имя));
  return совпало ? собратьИмяМеста(p, предел, различитель, true) : имя;
}
...
function перетаскиваниеСтрок(список, переставить){
  ...
  function строки(){
    // заголовки дней («День 1 · 40 км») стоят в списке, но это не точки
    return Array.prototype.filter.call(список.children, function(э){ return !э.classList.contains('drag-ph') && !э.classList.contains('dh'); });
  }
  function строкаИз(э){
  ...
  function конец(применить){
    ...
    for(var э = список.firstElementChild; э && э !== d.заместитель; э = э.nextElementSibling){
      if(э !== d.строка && !э.classList.contains('dh')) куда++;
    }
...
async function marshrutPage(ids, опции){
  ...
  if(изВидео){
    const сФото = ids.map(function(t){ return все.find(x => String(x.id) === t); })
      .filter(function(p){ return p && годенДляПревью(p.pic); })[0];
  ...
  return '<!doctype html><html lang="ru"><head><meta charset="utf-8">'
    ...
    // план дня, ночёвка и топливо
    + '.pl{margin:22px 0 0}.pl[hidden],.night[hidden],.nmsg[hidden],.nr-list[hidden],.go[hidden],a.go2[hidden],#rNightAll[hidden]{display:none}'
    + '.pl h2{font-size:17px;line-height:1.25;margin:0 0 8px;letter-spacing:-.01em}'
    + '.pl h3{font-size:15px;line-height:1.25;margin:14px 0 2px;color:#9a3412}'
    + '.plf{display:flex;flex-wrap:wrap;align-items:center;gap:8px 18px;margin:0 0 4px;font-size:15px}'
    + '.plf label{display:inline-flex;align-items:center;gap:8px}.plf label[hidden]{display:none}'
    + '#rNightL{flex:1 1 100%;min-width:0}#rNightL span{white-space:nowrap}#rNightL select{flex:1 1 auto;min-width:0;max-width:100%}'
    + '.pl input[type=time],.pl select,.ff input{font:inherit;color:inherit;background:#fff;border:1px solid #e9e2d8;border-radius:9px;padding:7px 9px}'
    + '.pl input[type=checkbox]{width:19px;height:19px;margin:0;accent-color:#9a3412}'
    + '.tw{overflow-x:auto}'
    + '.plt{width:100%;border-collapse:collapse;font-size:14.5px;line-height:1.25}'
    + '.plt th{font-size:12px;font-weight:600;color:#9c948c;text-align:left;padding:6px 6px 4px 0}'
    + '.plt td{padding:7px 6px 7px 0;border-top:1px solid #e9e2d8;vertical-align:middle}'
    + '.plt .pn{width:20px;color:#9a3412;font-weight:700}'
    + '.plt .tm{white-space:nowrap;font-weight:600;font-variant-numeric:tabular-nums}'
    + '.plt .tm small{display:block;font-weight:400;font-size:11px;color:#9c948c}'
    + '.plt select{font-size:14px;padding:5px 4px}'
    + '.plt tr.rd td{padding:6px 6px 0 0;font-size:12.5px;color:#9c948c}.plt tr.rd+tr.pp td{border-top:0;padding-top:3px}'
    + '.pi{margin:6px 0 0;font-size:15px}.pi small{color:#9c948c}'
    + '.dh{font-weight:700;font-size:14px;color:#9a3412;padding:12px 0 2px}.dh+.it{border-top:0}'
    + '.night{margin:16px 0 4px;padding:12px;border:1px solid #e9e2d8;border-radius:12px;background:#fff}'
    + '.night h3{margin:0 0 8px;color:#1c1917}'
    + '.nmsg{margin:0 0 6px;color:#57534e;font-size:14.5px}'
    + 'a.nc{text-decoration:none;color:inherit}a.nc .d{font-weight:700;color:#1c1917;margin:1px 0 0}'
    + '#rNightAll{display:inline-block;margin-top:8px;font-weight:600;font-size:14.5px}'
    + '.ff{display:flex;flex-wrap:wrap;align-items:flex-end;gap:10px}'
    + '.ff>div{flex:1 1 100px;display:flex;flex-direction:column;gap:4px;min-width:0}'
    + '.ff label{font-size:13px;line-height:1.2;color:#57534e}.ff input{width:100%}'
    + '.fs{margin:10px 0 0;font-size:18px;display:flex;flex-wrap:wrap;align-items:baseline;gap:4px 14px}'
    + '#rGo2{margin-left:10px}'
...
+ '.go2{display:inline-block;margin:18px 0 0 10px;background:#fff;border:1px solid #e9e2d8;color:#1c1917;'+   'text-decoration:none;font-weight:700;padding:13px 21px;border-radius:11px}'+ '.go2:hover{border-color:#9a3412;color:#9a3412}'
    ...
    // План дня, ночёвка и топливо тоже считаются в браузере: время выезда,
    // сколько пробыть и расход у каждого свои и лежат в localStorage.
    + '<section class="pl" id="rPlan" hidden aria-labelledby="rPlanH">'
    +   '<h2 id="rPlanH">План дня</h2>'
    +   '<div class="plf"><label>Выезд <input id="rStart" type="time" value="09:00"></label>'
    +     '<label id="rTwoL" hidden><input id="rTwo" type="checkbox"> Поездка на два дня</label>'
    +     '<label id="rNightL" hidden><span>Ночёвка после точки</span><select id="rNightN"></select></label></div>'
    +   '<div id="rPlan1"></div>'
    +   '<div class="night" id="rNight" hidden aria-live="polite"><h3 id="rNightH">Ночёвка</h3>'
    +     '<p class="nmsg" id="rNightMsg">Ищу жильё рядом…</p>'
    +     '<div class="nr-list" id="rNightList" hidden></div>'
    +     '<a id="rNightAll" href="/" hidden>Всё жильё рядом →</a></div>'
    +   '<div id="rPlan2"></div></section>'
    + '<section class="pl" id="rFuel" hidden aria-labelledby="rFuelH">'
    +   '<h2 id="rFuelH">Сколько стоит дорога</h2>'
    +   '<div class="ff">'
    +     '<div><label for="rFuelKm">Расстояние, км</label><input id="rFuelKm" type="text" inputmode="decimal" autocomplete="off"></div>'
    +     '<div><label for="rFuelUse">Расход, л на 100 км</label><input id="rFuelUse" type="text" inputmode="decimal" autocomplete="off" value="7.5"></div>'
    +     '<div><label for="rFuelPrice">Цена литра, BYN</label><input id="rFuelPrice" type="text" inputmode="decimal" autocomplete="off" value="2.60"></div>'
    +   '</div>'
    +   '<p class="fs"><b id="rFuelSum">—</b><button class="auto" id="rFuelAuto" type="button" hidden>↺ по маршруту</button></p></section>'
    ...
    // пришли минуты и километры по дорогам — план и топливо пересчитываем по ним
    +     'планИТопливо();}'
    ...
    // Маршрут из видео заменили точками из другой вкладки — заголовок и
    // вступление ролика над чужими точками только путают: страница
    // становится обычным «Маршрутом на день». Свои правки здесь же их не трогают.
    ...
    +     'строки:(function(){var с=Т.map(function(p,i){return {номер:i+1,название:p.name,адрес:p.addr||"",своя:своя(p)};});'
    // поездка на два дня — в списке на картинке те же дни, что на странице
    +       'if(ДВА_ДНЯ&&Т.length>2){с.splice(НОЧЁВКА,0,{разделитель:"День 2"});с.unshift({разделитель:"День 1"});}return с;})(),'
    ...
    // лишних запросов: минуты и километры перегонов — из уже полученной дороги.
    + 'var ПРОБЫТЬ_ВАРИАНТЫ=[15,30,45,60,90,120,180];'
    + 'function читатьОбъект(к){try{var v=JSON.parse(localStorage.getItem(к)||"null");'
    +   'return (v&&typeof v==="object"&&!Array.isArray(v))?v:{};}catch(e){return {};}}'
    + 'function записатьОбъект(к,v){try{localStorage.setItem(к,JSON.stringify(v));}catch(e){}}'
    + 'var ПРОБЫТЬ=читатьОбъект("routeStay");'
    // Два дня и ночёвка — часть ссылки (d=N), а не настройка человека: время
    ...
    + 'function время(м){м=Math.round(м);var д=Math.floor(м/1440),r=м-д*1440;'
    +   'return ("0"+Math.floor(r/60)).slice(-2)+":"+("0"+(r%60)).slice(-2)'
    +     '+(д?(" <small>+"+д+" "+(д===1?"день":д<5?"дня":"дней")+"</small>"):"");}'
    + 'function длительность(м){var ч=Math.floor(м/60),r=м%60;return ч?(ч+" ч"+(r?(" "+r+" мин"):"")):(м+" мин");}'
    + 'function пробыть(id){var v=+ПРОБЫТЬ[String(id)];return ПРОБЫТЬ_ВАРИАНТЫ.indexOf(v)>=0?v:45;}'
    // Перегоны: км и минуты по дорогам, если дорога посчитана для этого порядка
    // точек; иначе по прямой ×1,3 при 60 км/ч — с пометкой «примерно».
    + 'function перегоны(){var d=(ДОРОГА&&ДОРОГА.к===ключДороги())?ДОРОГА.d:null, out=[];'
    +   'for(var i=1;i<Т.length;i++){var прямо=км(Т[i-1],Т[i])*1.3;'
    +     'var к=(d&&Array.isArray(d.legs)&&typeof d.legs[i-1]==="number")?d.legs[i-1]:null;'
    ...
    +   'var q=location.search.replace(/^[?]/,"").split("&").filter(function(x){return x&&x.indexOf("d=")!==0;});'
    +   'if(ДВА_ДНЯ)q.push("d="+НОЧЁВКА);'
    +   'history.replaceState(null,"",location.pathname+(q.length?("?"+q.join("&")):"")+location.hash);}'
    + 'function поправитьДни(){var n=Т.length, было=ДВА_ДНЯ?НОЧЁВКА:0;'
    // точек стало меньше трёх — делить нечего: выключаем и убираем d из адреса
    +   'if(ДВА_ДНЯ&&n<3){ДВА_ДНЯ=false;НОЧЁВКА=0;НОЧЁВКА_САМА=true;адресДней();return;}'
    +   'if(!ДВА_ДНЯ)return;'
    +   'if(НОЧЁВКА_САМА)НОЧЁВКА=ночёвкаПоУмолчанию();'
    +   'НОЧЁВКА=Math.max(1,Math.min(n-1,НОЧЁВКА||1));'
    +   'if(НОЧЁВКА!==было)адресДней();}'
    + 'function таблицаДня(a,b,п,день){var t=минутыВыезда(), пр=false, строки="";'
    +   'for(var i=a;i<=b;i++){'
    // второй день начинается от ночёвки у последней точки первого: этот перегон — уже его
    +     'if(i>0&&(i>a||день===2)){var л=п[i-1];t+=л.мин;if(л.примерно)пр=true;'
    +       'строки+="<tr class=\\"rd\\"><td></td><td colspan=\\"4\\">↓ "+длительность(л.мин)+" в дороге"'
    +         '+(i===a?(" от точки "+i):"")+(л.примерно?" · примерно":"")+"</td></tr>";}'
    +     'var p=Т[i], сколько=пробыть(p.id), приезд=t, метка=пр?"<small>примерно</small>":""; t+=сколько;'
    +     'строки+="<tr class=\\"pp\\" data-id=\\""+esc(p.id)+"\\"><td class=\\"pn\\">"+(i+1)+"</td><td>"+esc(p.name)+"</td>"'
    +       '+"<td class=\\"tm ar\\">"+время(приезд)+метка+"</td>"'
    +       '+"<td><select class=\\"ps\\" data-id=\\""+esc(p.id)+"\\" aria-label=\\"Сколько пробыть: "+esc(p.name)+"\\">"'
    +       '+ПРОБЫТЬ_ВАРИАНТЫ.map(function(v){return "<option value=\\""+v+"\\""+(v===сколько?" selected":"")+">"'
    +         '+(v<60?(v+" мин"):(String(v/60).replace(".",",")+" ч"))+"</option>";}).join("")+"</select></td>"'
    +       '+"<td class=\\"tm dp\\">"+время(t)+метка+"</td></tr>";}'
    +   'return "<div class=\\"tw\\"><table class=\\"plt\\"><thead><tr><th></th><th>Точка</th><th>Приезд</th><th>Пробыть</th><th>Отъезд</th></tr></thead>"'
    +     '+"<tbody>"+строки+"</tbody></table></div>"'
    +     '+"<p class=\\"pi\\">Последняя точка — до <b>"+время(t)+"</b>"+(пр?" <small>примерно</small>":"")+"</p>";}'
    + 'function яндекс(с){return "https://yandex.by/maps/?rtext="+с.map(function(p){return p.lat+","+p.lng;}).join("~")+"&rtt=auto";}'
    + 'function яндексПоДням(N){var g=document.getElementById("rGo"),g1=document.getElementById("rGo1"),g2=document.getElementById("rGo2");'
    +   'g.hidden=ДВА_ДНЯ; g1.hidden=!ДВА_ДНЯ; g2.hidden=!ДВА_ДНЯ; if(!ДВА_ДНЯ)return;'
    // второй день едет от ночёвки у последней точки первого
    +   'g1.href=яндекс(Т.slice(0,N)); g2.href=яндекс(Т.slice(N-1));}'
    // Заголовки дней вставляем в готовый список, а не собираем в нарисовать():
    // включение двух дней не должно перерисовывать карту и дёргать масштаб.
    + 'function заголовкиДней(км1,км2){var л=document.getElementById("rlist");'
    +   'Array.prototype.slice.call(л.querySelectorAll(".dh")).forEach(function(э){if(э.parentNode===л)л.removeChild(э);});'
    +   'if(!ДВА_ДНЯ)return; var ряды=л.querySelectorAll(".it"); if(ряды.length!==Т.length)return;'
    +   '[[0,"День 1 · "+Math.round(км1)+" км"],[НОЧЁВКА,"День 2 · "+Math.round(км2)+" км"]].forEach(function(х){'
    +     'var h=document.createElement("div");h.className="dh";h.textContent=х[1];л.insertBefore(h,ряды[х[0]]);});}'
    + 'var подписьНочёвки="";'
    + 'function планИТопливо(){var n=Т.length;'
    +   'document.getElementById("rPlan").hidden=n<2; document.getElementById("rFuel").hidden=n<2;'
    +   'поправитьДни();'
    +   'document.getElementById("rTwoL").hidden=n<3; document.getElementById("rTwo").checked=ДВА_ДНЯ;'
    +   'document.getElementById("rNightL").hidden=!ДВА_ДНЯ;'
    +   'var п=перегоны(), N=ДВА_ДНЯ?НОЧЁВКА:n, км1=0, км2=0;'
    +   'п.forEach(function(x,i){if(i<N-1)км1+=x.км;else км2+=x.км;});'
    +   'var д1=document.getElementById("rPlan1"), д2=document.getElementById("rPlan2");'
    +   'if(n<2){д1.innerHTML="";д2.innerHTML="";}'
    +   'else if(ДВА_ДНЯ){д1.innerHTML="<h3>День 1 · "+Math.round(км1)+" км</h3>"+таблицаДня(0,N-1,п,1);'
    +     'д2.innerHTML="<h3>День 2 · "+Math.round(км2)+" км</h3>"+таблицаДня(N,n-1,п,2);}'
    +   'else{д1.innerHTML=таблицаДня(0,n-1,п,1);д2.innerHTML="";}'
    +   'if(ДВА_ДНЯ){var s=document.getElementById("rNightN"), подпись=ключИд(Т);'
    +     'if(подпись!==подписьНочёвки){подписьНочёвки=подпись;'
    +       's.innerHTML=Т.slice(0,n-1).map(function(p,i){var имя=String(p.name||"");if(имя.length>28)имя=имя.slice(0,27)+"…";'
    +         'return "<option value=\\""+(i+1)+"\\">"+(i+1)+" · "+esc(имя)+"</option>";}).join("");}'
    +     's.value=String(N);}'
    +   'заголовкиДней(км1,км2); яндексПоДням(N); ночлег(); топливо();}'
    // Жильё у ночёвки. Спрашиваем только при включённых двух днях, только для
    // выбранной точки и через 600 мс после последней смены — поиск идёт по всей округе.
    + 'var ЖИЛЬЁ={}, жильёКлюч="", жильёЗа=null;'
    + 'function сообщениеЖилья(т){var м=document.getElementById("rNightMsg"),л=document.getElementById("rNightList");'
    +   'м.textContent=т;м.hidden=false;л.hidden=true;л.innerHTML="";document.getElementById("rNightAll").hidden=true;}'
    + 'function ночлег(){var б=document.getElementById("rNight");'
    +   'if(!ДВА_ДНЯ){б.hidden=true;clearTimeout(жильёЗа);жильёКлюч="";return;}'
    +   'var p=Т[НОЧЁВКА-1], к=p.lat+","+p.lng;'
    +   'document.getElementById("rNightH").textContent="Ночёвка рядом с «"+p.name+"»";'
    +   'б.hidden=false;'
    +   'if(к===жильёКлюч)return; жильёКлюч=к; clearTimeout(жильёЗа);'
    +   'if(ЖИЛЬЁ[к]){показатьЖильё(ЖИЛЬЁ[к]);return;}'
    +   'сообщениеЖилья("Ищу жильё рядом…");'
    ...
    +   'if(!items.length)сообщениеЖилья("В 30 км жилья не нашлось");'
    +   'else{document.getElementById("rNightMsg").hidden=true; л.hidden=false;'
    // сначала показываем ленту, потом вставляем карточки: ленивые снимки в скрытом блоке не грузятся
    +     'л.innerHTML=items.map(function(x){var ф=x.photos&&x.photos[0];'
    +       'return "<a class=\\"nc\\" href=\\""+esc(x.link)+"\\" target=\\"_blank\\" rel=\\"noopener\\">"'
    +         '+(ф?("<img src=\\""+esc(ф)+"\\" alt=\\"\\" loading=\\"lazy\\">"):"<div class=\\"ni\\"></div>")'
    +         '+"<span class=\\"p\\">"+esc(x.title||x.name||"Жильё на сутки")+"</span>"'
    +         '+"<span class=\\"d\\">"+(x.от?"от ":"")+esc(x.price)+" BYN</span>"'
    ...
    + 'function число(v){v=String(v==null?"":v).trim().replace(",",".");if(!v)return NaN;var x=Number(v);return (isFinite(x)&&x>=0)?x:NaN;}'
    + 'function расстояниеМаршрута(){if(Т.length<2)return 0;var d=(ДОРОГА&&ДОРОГА.к===ключДороги())?ДОРОГА.d:null;'
    +   'if(d&&typeof d.km==="number")return Math.round(d.km);'
    +   'var s=0;Т.forEach(function(p,i){if(i)s+=км(Т[i-1],p);});return Math.round(s*1.3);}'
    + 'function топливо(){if(РАССТ_САМО){var в=расстояниеМаршрута();document.getElementById("rFuelKm").value=в?String(в):"";}'
    +   'document.getElementById("rFuelAuto").hidden=РАССТ_САМО; посчитатьТопливо();}'
    + 'function посчитатьТопливо(){var р=число(document.getElementById("rFuelKm").value), л=число(document.getElementById("rFuelUse").value),'
    +   'ц=число(document.getElementById("rFuelPrice").value), о=document.getElementById("rFuelSum");'
    ...
    +   'var id=t.getAttribute("data-id");ПРОБЫТЬ[id]=+t.value;'
    // в хранилище дописываем к тому, что там уже есть: в другой вкладке могли выбрать своё
    +   'var все=читатьОбъект("routeStay");все[id]=+t.value;записатьОбъект("routeStay",все);'
    +   'планИТопливо();'
    ...
    +   'безВидео(н); ПОРЯДОК=прочитатьПорядок(); Т=н; var был=СВЯЗАН; СВЯЗАН=true; нарисовать(); if(!был)обновитьАдрес();});'
    ...
    +   'if(!СВЯЗАН){if(!маршрутМенялся||!н.length)return; безВидео(н); Т=н; ПОРЯДОК=п; СВЯЗАН=true; нарисовать(); обновитьАдрес(); return;}'
    ...
    +   'if(!другие&&п===ПОРЯДОК)return; ПОРЯДОК=п; if(другие){безВидео(н);Т=н;} нарисовать();});' + '</' + 'script>'
    + '</div></body></html>';
}
...
async function видеоСписокPage(){
  ...
  const карточки = ВИДЕО_МАРШРУТЫ.map(function(м, i){
    ...
    if(!обложка){ const годный = т.find(p => годенДляПревью(p.pic)); if(годный) обложка = годный.pic; }
...
async function гидДанные(z){
  const по = {};
  let полный = true;   // все запросы прошли и площадки ответили (см. ответПолный)
  for(const в of ВИДЫ){
    const п = new URL('/api/search?region=' + z.обл + '&city=' + encodeURIComponent(z.город) +
                      '&type=' + в.код + '&source=both', 'http://localhost');
    let d = { items: [], total: 0 };
    try{ d = await runSearchQuery(п.searchParams); }catch(e){ полный = false; }
    // Квартиры в городе есть у каждой площадки — по ним и видно, ответила ли она.
    // У усадеб и коттеджей ноль от Realt бывает и честным.
    if(в.код === 'flat' && !ответПолный(d)) полный = false;
  ...
  return { по: по, районы: список, места: места, полный: полный };
}

// Гиды и страницы спроса, которые при сборке оказались пустыми и отдали 404:
// адрес → когда это выяснилось. В sitemap их не кладём: поисковик иначе
// получает из карты сайта битые адреса (вариантов в Уручье то пять, то
// четыре). Узнаём только при сборке страницы — ради карты сайта в источники
// жилья не ходим.
// Две защиты от ложной «пустоты». Помечаем, только если площадки правда
// ответили (ответПолный): после холодного старта или при отказе Kufar ноль
// вариантов ничего не говорит о спросе, и живая страница выпала бы из карты
// сайта. И пометка живёт час: дальше адрес снова в sitemap, пока его не
// соберут заново.
const ПУСТЫЕ_СТРАНИЦЫ = new Map();
const ПУСТАЯ_ЖИВЁТ = 60 * 60 * 1000;
function пустаяСтраница(slug){
  const когда = ПУСТЫЕ_СТРАНИЦЫ.get(slug);
  if(!когда) return false;
  if(Date.now() - когда < ПУСТАЯ_ЖИВЁТ) return true;
  ПУСТЫЕ_СТРАНИЦЫ.delete(slug);
  return false;
}
// Площадки глотают свои сбои и отдают пустой список, так что «0 вариантов»
// сам по себе неотличим от отказа. Ответ считаем полным, когда каждая
// включённая живая площадка (Kufar, Realt) что-то вернула по общему запросу
// города или области, а каталоги Check-in и Kvartirka уже собраны. Лучше
// оставить в sitemap пустую страницу, чем выкинуть живую.
function ответПолный(d){
  if(!d) return false;
  if(ИСТОЧНИКИ.kufar && !(d.kufar > 0)) return false;
  if(ИСТОЧНИКИ.realt && !(d.realt > 0)) return false;
  if(ИСТОЧНИКИ.checkin && !КАТАЛОГ_ОБНОВЛЁН.CheckIn) return false;
...
async function гидPage(slug){
  ...
  if(всего < 20){ if(d.полный) ПУСТЫЕ_СТРАНИЦЫ.set(slug, Date.now()); return ''; }
  ПУСТЫЕ_СТРАНИЦЫ.delete(slug);
...
async function спросДанные(z){
  if(z.точка){
    ...
    // stayNearPoint сбоев не показывает — площадки проверяем по квартирам
    // области вокруг точки (тот же запрос лежит в кэше после индекса «рядом»)
    let обл = null;
    try{ обл = await runSearchQuery(new URLSearchParams({ region: nearestRegion(z.точка[0], z.точка[1]), city:'', type:'flat',
...
async function спросPage(slug){
  ...
  if(d.total < СПРОС_МИНИМУМ){ if(d.полный) ПУСТЫЕ_СТРАНИЦЫ.set(slug, Date.now()); return ''; }
  ПУСТЫЕ_СТРАНИЦЫ.delete(slug);
...
const ВКЛАДКИ_ГЛАВНОЙ = {
  ...
  ru: { путь: '/?country=ru', title: 'Отели и жильё в России на сутки: цены 101Hotels на карте',
        desc: 'Отели, апартаменты, гостевые дома и хостелы в ' + Object.keys(RF_CITIES).length + ' городах и курортах России: Москва, Петербург, '
...
/* На узком экране широкая полоса обрезается до куска стены — там свой кадр:
   обе башни целиком. Полоса там шире снимка по пропорциям (до 700 px) —
   режется низ, а не кресты наверху */
...
/* Ссылка лежит поверх фотографии в шапке, а она не CSS-фон, а картинка —
   подобрать «светлый или тёмный» текст нельзя. Поэтому небольшая плашка:
   читается и на снимке, и на светлом фоне, если шапку когда-нибудь уберут. */
...
<div class="wrap">
 <div class="hero-ink">
  ...
  <a class="knizu" href="#foot" id="knizu">Сдаёте жильё? Есть пожелания проекту? Напишите нам ↓</a>
...
const ЗАГОЛОВКИ_ВКЛАДОК = ${вСкрипт({ by: ВКЛАДКИ_ГЛАВНОЙ.by.title, ru: ВКЛАДКИ_ГЛАВНОЙ.ru.title, places: ВКЛАДКИ_ГЛАВНОЙ.places.title })};
const PAGE_SIZE = 24;
window.__page = 1;
window.__view = 'list';
...
// Прокрутку к форме связи делает сама ссылка-якорь: у страницы включено
// scroll-behavior:smooth, поэтому переход к #foot и так плавный. Свой
// обработчик тут был лишним — только добавлял способ сломаться.
...
http.createServer(async (req,res)=>{
  ...
  // Пометки пустых гидов и страниц спроса (только DATA_TEST=1): GET — что
  // помечено и сколько лет пометке; POST ?slug=…&age=мс — поставить пометку
  // «давностью age», чтобы проверить час жизни без часа ожидания.
  if(process.env.DATA_TEST === '1' && u.pathname === '/api/_empty-test'){
    if(req.method === 'POST'){
      const slug = u.searchParams.get('slug') || '';
      if(ГИДЫ[slug] || СПРОС[slug]) ПУСТЫЕ_СТРАНИЦЫ.set(slug, Date.now() - (+u.searchParams.get('age') || 0));
    }
    const пометки = {};
    ПУСТЫЕ_СТРАНИЦЫ.forEach(function(t, k){ пометки[k] = Date.now() - t; });
    res.writeHead(200, {'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store'});
    res.end(JSON.stringify({ pid: process.pid, живёт: ПУСТАЯ_ЖИВЁТ, пометки })); return;
  }
  ...
  // GET ?имя=… — что сейчас в памяти у подключённого имени (или в файле).
  if(process.env.DATA_TEST === '1' && u.pathname === '/api/_data-test' && req.method === 'GET'){
    const имя = u.searchParams.get('имя') || '';
    const объект = Object.prototype.hasOwnProperty.call(ДАННЫЕ_ПРОВЕРКИ, имя)
      ? ДАННЫЕ_ПРОВЕРКИ[имя] : прочитатьДанные(имя, null);
    res.writeHead(200, {'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store'});
    res.end(JSON.stringify({объект, основа: ОСНОВЫ_ПРОВЕРКИ[имя]})); return;
  }
  if(process.env.DATA_TEST === '1' && u.pathname === '/api/_data-test' && req.method === 'POST'){
    ...
    req.on('end', ()=>{
      ...
      try{
        ...
        // {подключить: имя, вид} — подключить сейчас: так проверка сама
        // выбирает момент загрузки и не зависит от того, как долго сервер вставал.
        if(d.подключить){
          ok = можноИмя(d.подключить);
          if(ok) подключитьПроверку(d.подключить, d.вид || 'обычное');
        }else{
          ok = записатьДанные(d.имя, d.объект);
        }
        // как настоящий потребитель: данные в памяти меняются вместе с файлом
        if(ok && Object.prototype.hasOwnProperty.call(ДАННЫЕ_ПРОВЕРКИ, d.имя)) ДАННЫЕ_ПРОВЕРКИ[d.имя] = d.объект;
      }catch(e){}
  ...
  if(u.pathname === '/api/t' && req.method === 'POST'){
    ...
    req.on('end', ()=>{
      try{
        ...
        if(d.e === 'route_add') голосЗаМесто(d.id, String(d.v||'').slice(0,32), botOf(ua), адресКлиента(req)).catch(function(){});
      }catch(e){}
  ...
  if(u.pathname === '/istochnik'){
    ...
    const каталоги = ['CheckIn', 'Kvartirka'].map(function(и){
      ...
      // Сколько меток не сошлось с адресом (см. точкаОбъявления): рост этого
      // числа — знак, что площадка сломала координаты, а не хозяева ошиблись.
      const вЦентр = КАТАЛОГ[и].filter(function(x){ return /^в центр/.test(x.сверено || ''); }).length;
      const убрано = КАТАЛОГ[и].filter(function(x){ return x.сверено === 'убрано с карты'; }).length;
      return '  ' + и.padEnd(10) + String(КАТАЛОГ[и].length).padStart(5) + ' объявлений'
        + (когда ? (', обновлён ' + мин + ' мин назад') : ', ещё не собирался')
        + ((вЦентр || убрано) ? ('\n             метка не у адреса: в центр города ' + вЦентр + ', убрано с карты ' + убрано) : '')
  ...
  if(u.pathname === '/api/places'){
    ...
    if(q){
      ...
      const score = p => {
        // alt — другие написания, которых нет в названии и адресе: деревню Осово
        // пишут и «Осова», и «Оссово», и по-белорусски «Асава». В карточке их не показываем.
        const n = norm(p.name), a = norm(p.addr) + ' ' + norm(p.cat) + ' ' + norm(p.alt);
        if(n === q) return 0;
  ...
  if(u.pathname === '/sitemap.xml'){
    ...
    urls.push.apply(urls, Object.keys(ГИДЫ).filter(k => !пустаяСтраница(k)).map(function(k){
